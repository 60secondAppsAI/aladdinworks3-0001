					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateMonitoringSession()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2MonitoringSessions = true">Add</base-button>
								              <modal :show.sync="modal2MonitoringSessions">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Monitoring Session Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="MonitoringSessionId" type="text" placeholder="Enter MonitoringSessionId" v-model="monitoringSessionDetails.monitoringSessionId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Start Date Time</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="StartDateTime" type="text" placeholder="Enter StartDateTime" v-model="monitoringSessionDetails.startDateTime"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">End Date Time</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="EndDateTime" type="text" placeholder="Enter EndDateTime" v-model="monitoringSessionDetails.endDateTime"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      ...
      modal2MonitoringSessions: false,
      monitoringSessionDetails: null,
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2MonitoringSessions = false;
    ...
    async updateMonitoringSession() {

      const jsonData = JSON.stringify(this.monitoringSessionDetails);
      const res = await MonitoringSessionService.update(jsonData);
...
//        this.getAllMonitoringSessions();
    ...
    async getMonitoringSessionDetails() {
      try {
			let response = await MonitoringSessionService.get(this.monitoringSessionId);
			this.monitoringSessionDetails = response.data;
 ...
 formatDate(dateString) {
  ...
  mounted() {
    this.getMonitoringSessionDetails();
  },
  computed: {
  },
