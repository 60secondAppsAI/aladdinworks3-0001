        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Power Meters</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalPowerMeters = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPowerMeters">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add PowerMeter</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="PowerMeterId" type="text" placeholder="Enter PowerMeterId" v-model="powerMeterToAdd.powerMeterId"></base-input>
  <base-input label="Power" type="text" placeholder="Enter Power" v-model="powerMeterToAdd.power"></base-input>
  <base-input label="Location" type="text" placeholder="Enter Location" v-model="powerMeterToAdd.location"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="powerMeters" :row-key="record => record.PowerMeterId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PowerMeterPictureView :powerMeters="powerMeters" />
          </div>
...
import PowerMeterPictureView from './PowerMeterPictureView.vue';


const powerMetersColumns = [
  "powerMeterId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        {
          title: 'Power Meter Id',
		dataIndex: 'powerMeterId',
          visible: true,
          scopedSlots: { customRender: 'powerMeterId' },
          sorter: true
          //sorter: (a, b) => a.powerMeterId - b.powerMeterId,
          //sorter: (a, b) => a.powerMeterId.localeCompare(b.powerMeterId),
        },
        {
          title: 'Power',
		dataIndex: 'power',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} powerMeters`,
      },
      ...
      powerMetersTable: {
        columns: [...powerMetersColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'powerMeterId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPowerMetersTable() {
      ...
      let powerMetersTableData = [];
      for (let i = 0; i < this.powerMeters.length; i++) {
        powerMetersTableData.push({
          id: i,
          powerMeterId: this.powerMeters[i].powerMeterId,
          year: this.powerMeters[i].year,
          date: this.powerMeters[i].date,
          competitionId: this.powerMeters[i].competitionId,
          powerMeterId: this.powerMeters[i].powerMeterId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-power-meters',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPowerMetersChanged', this.searchQuery);
		//this.renderPowerMetersTable();
    },
	...
	async handleAddSubmitted() {
      this.modalPowerMeters = false;

      const currentDate = new Date().getTime();
      this.powerMeterToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.powerMeterToAdd);
      console.log(jsonData);
      
      const res = await PowerMeterService.addPowerMeter(jsonData);
