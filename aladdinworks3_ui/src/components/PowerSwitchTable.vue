        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Power Switchs</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalPowerSwitchs = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPowerSwitchs">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add PowerSwitch</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="PowerSwitchId" type="text" placeholder="Enter PowerSwitchId" v-model="powerSwitchToAdd.powerSwitchId"></base-input>
  <base-input label="State" type="text" placeholder="Enter State" v-model="powerSwitchToAdd.state"></base-input>
  <base-input label="ControlVoltage" type="text" placeholder="Enter ControlVoltage" v-model="powerSwitchToAdd.controlVoltage"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="powerSwitchs" :row-key="record => record.PowerSwitchId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PowerSwitchPictureView :powerSwitchs="powerSwitchs" />
          </div>
...
import PowerSwitchPictureView from './PowerSwitchPictureView.vue';


const powerSwitchsColumns = [
  "powerSwitchId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    PowerSwitchPictureView
  },

  data() {
    return {
      modalPowerSwitchs: false,
        isTableView: true,

      columns: [
        {
          title: 'Power Switch Id',
		dataIndex: 'powerSwitchId',
          visible: true,
          scopedSlots: { customRender: 'powerSwitchId' },
          sorter: true
          //sorter: (a, b) => a.powerSwitchId - b.powerSwitchId,
          //sorter: (a, b) => a.powerSwitchId.localeCompare(b.powerSwitchId),
        },
        {
          title: 'State',
		dataIndex: 'state',
		...
		dataIndex: 'controlVoltage',
          visible: true,
          scopedSlots: { customRender: 'controlVoltage' },
          sorter: true
          //sorter: (a, b) => a.controlVoltage - b.controlVoltage,
          //sorter: (a, b) => a.controlVoltage.localeCompare(b.controlVoltage),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} powerSwitchs`,
      },
      ...
      powerSwitchsTable: {
        columns: [...powerSwitchsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'powerSwitchId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPowerSwitchsTable() {
      ...
      let powerSwitchsTableData = [];
      for (let i = 0; i < this.powerSwitchs.length; i++) {
        powerSwitchsTableData.push({
          id: i,
          powerSwitchId: this.powerSwitchs[i].powerSwitchId,
          year: this.powerSwitchs[i].year,
          date: this.powerSwitchs[i].date,
          competitionId: this.powerSwitchs[i].competitionId,
          powerSwitchId: this.powerSwitchs[i].powerSwitchId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-power-switchs',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPowerSwitchsChanged', this.searchQuery);
		//this.renderPowerSwitchsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalPowerSwitchs = false;

      const currentDate = new Date().getTime();
      this.powerSwitchToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.powerSwitchToAdd);
      console.log(jsonData);
      
      const res = await PowerSwitchService.addPowerSwitch(jsonData);
