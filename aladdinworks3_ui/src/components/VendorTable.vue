        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalVendors = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalVendors">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Vendor</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="VendorId" type="text" placeholder="Enter VendorId" v-model="vendorToAdd.vendorId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="vendorToAdd.name"></base-input>
  <base-input label="ContactInfo" type="text" placeholder="Enter ContactInfo" v-model="vendorToAdd.contactInfo"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="vendors" :row-key="record => record.VendorId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <VendorPictureView :vendors="vendors" />
          </div>
...
import VendorPictureView from './VendorPictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'vendorId',
          visible: true,
          scopedSlots: { customRender: 'vendorId' },
          sorter: true
      ...
      sortBy: 'vendorId',           // Column to sort by
  ...
  methods: {
    ...
    async renderVendorsTable() {
      ...
      for (let i = 0; i < this.vendors.length; i++) {
        vendorsTableData.push({
          id: i,
          vendorId: this.vendors[i].vendorId,
          year: this.vendors[i].year,
          date: this.vendors[i].date,
          competitionId: this.vendors[i].competitionId,
          vendorId: this.vendors[i].vendorId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-vendors',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForVendorsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalVendors = false;

      const currentDate = new Date().getTime();
      this.vendorToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.vendorToAdd);
      console.log(jsonData);
      
      const res = await VendorService.addVendor(jsonData);
