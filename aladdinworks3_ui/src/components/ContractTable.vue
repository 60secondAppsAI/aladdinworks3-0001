...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalContracts = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalContracts">
                <template slot="header">
  ...
  <base-input label="Terms" type="text" placeholder="Enter Terms" v-model="contractToAdd.terms"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="contracts" :row-key="record => record.ContractId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ContractPictureView :contracts="contracts" />
          </div>
...
import ContractPictureView from './ContractPictureView.vue';
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'terms',
          ...
          //sorter: (a, b) => a.terms.localeCompare(b.terms),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'contractId',           // Column to sort by
  ...
  methods: {
    ...
    async renderContractsTable() {
      ...
      for (let i = 0; i < this.contracts.length; i++) {
        contractsTableData.push({
          id: i,
          contractId: this.contracts[i].contractId,
          year: this.contracts[i].year,
          date: this.contracts[i].date,
          competitionId: this.contracts[i].competitionId,
          contractId: this.contracts[i].contractId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-contracts',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForContractsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.contractToAdd.created = currentDate;
