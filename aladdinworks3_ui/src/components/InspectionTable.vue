        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalInspections = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalInspections">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Inspection</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="InspectionId" type="text" placeholder="Enter InspectionId" v-model="inspectionToAdd.inspectionId"></base-input>
  <base-input label="Date" type="text" placeholder="Enter Date" v-model="inspectionToAdd.date"></base-input>
  <base-input label="InspectorName" type="text" placeholder="Enter InspectorName" v-model="inspectionToAdd.inspectorName"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="inspections" :row-key="record => record.InspectionId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <InspectionPictureView :inspections="inspections" />
          </div>
...
import InspectionPictureView from './InspectionPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    InspectionPictureView
  },

  data() {
    return {
      modalInspections: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'inspectionId',
          visible: true,
          scopedSlots: { customRender: 'inspectionId' },
          sorter: true
          //sorter: (a, b) => a.inspectionId - b.inspectionId,
          //sorter: (a, b) => a.inspectionId.localeCompare(b.inspectionId),
        },
        {
          title: 'Date',
		dataIndex: 'date',
		...
		dataIndex: 'inspectorName',
          ...
          sorter: true
          //sorter: (a, b) => a.inspectorName - b.inspectorName,
          //sorter: (a, b) => a.inspectorName.localeCompare(b.inspectorName),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'inspectionId',           // Column to sort by
  ...
  methods: {
    ...
    async renderInspectionsTable() {
      ...
      let inspectionsTableData = [];
      for (let i = 0; i < this.inspections.length; i++) {
        inspectionsTableData.push({
          id: i,
          inspectionId: this.inspections[i].inspectionId,
          year: this.inspections[i].year,
          date: this.inspections[i].date,
          competitionId: this.inspections[i].competitionId,
          inspectionId: this.inspections[i].inspectionId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-inspections',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForInspectionsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalInspections = false;

      const currentDate = new Date().getTime();
      this.inspectionToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.inspectionToAdd);
      console.log(jsonData);
      
      const res = await InspectionService.addInspection(jsonData);
